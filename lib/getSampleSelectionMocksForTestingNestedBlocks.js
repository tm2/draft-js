...
function getSampleSelectionMocksForTestingNestedBlocks() {
  ...
  var text = ['Washington', '', 'Lincoln', 'Kennedy'];
  ...
  var aChars = Immutable.List(Immutable.Repeat(boldChar, text[0].length));
  var bChars = Immutable.List(Immutable.Repeat(EMPTY, text[1].length));
  var cChars = Immutable.List(Immutable.Repeat(boldChar, text[2].length));
  var dChars = Immutable.List(Immutable.Repeat(EMPTY, text[3].length));
  ...
  leafs = ['a-0-0', 'b-0-0', 'b/c-0-0', 'b/d-0-0'].map(function (blockKey, ii) {
    var span = document.createElement('span');
    span.setAttribute('data-offset-key', '' + blockKey);
    span.appendChild(leafChildren[ii]);
    return span;
  });
  decorators = ['a-0-0', 'b-0-0', 'b/c-0-0', 'b/d-0-0'].map(function (decoratorKey, ii) {
    var span = document.createElement('span');
    span.setAttribute('data-offset-key', '' + decoratorKey);
    span.appendChild(leafs[ii]);
    return span;
  });
  blocks = ['a-0-0', 'b-0-0', 'b/c-0-0', 'b/d-0-0'].map(function (blockKey, ii) {
  ...
  blocks.forEach(function (blockElem, index, arr) {
    var currentBlock = contentBlocks[index];
    var parentKey = currentBlock.getParentKey();
    var hasChildren = contentState.getBlockChildren(currentBlock.getKey()).size > 0;

    if (hasChildren) {
      // if a block has children it should not have leafs just a data-blocks container
      var dataBlocks = document.createElement('div');
      dataBlocks.setAttribute('data-blocks', 'true');
      blockElem.firstChild.replaceChild(dataBlocks, blockElem.querySelector('span'));
    }

    if (parentKey) {
      var parentIndex = blockKeys.indexOf(parentKey);
      var parentBlockElement = arr[parentIndex];
      var blockContainer = parentBlockElement.querySelector('div[data-blocks]');
